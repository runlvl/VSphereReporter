...
def collect_orphaned_vmdks(client):
    ...
    try:
                        ...
                        try:
                            ...
                            if hasattr(device.backing, 'fileName') and device.backing.fileName:
                                ...
                                disk_info = {
                                    ...
                                    'status': "TEMPLATE" if is_template else "AKTIV",
                                    'explanation': "TEMPLATE-VMDK: Diese VMDK-Datei gehört zu einer Template-VM." if is_template 
                                                 else "AKTIVE VMDK: Diese VMDK wird von einer aktiven VM verwendet."
                                }
                                ...
                                try:
                                    ...
                                    if is_orphaned:
                                        ...
                                        orphaned_info = {
                                            ...
                                            'explanation': "POTENTIALLY ORPHANED: Diese VMDK-Datei scheint keiner VM zugeordnet zu sein. Bitte überprüfen Sie die Datei manuell."
                                        }
        ...
        # NEUE STRATEGIE: ZEIGE ALLE VMDKs und markiere potentiell verwaiste
        # Anstatt nur die potentiell verwaisten VMDKs zurückzugeben, zeigen wir jetzt
        # ALLE VMDKs und markieren diejenigen, die potentiell verwaist sind.
        
        # Kombinierte Liste aus allen gefundenen VMDKs zurückgeben (aktive und potentiell verwaiste)
        logger.info(f"NEU: Zeige alle VMDKs ({len(orphaned_and_used_vmdks)}) und markiere potentiell verwaiste ({len(orphaned_vmdks)})")
        
        # Füge alle VMDK-Dateien zusammen und markiere Status
        final_vmdk_list = orphaned_and_used_vmdks.copy()
        
        # Gibt die vollständige Liste mit allen VMDKs zurück - aktive und potentiell verwaiste
        return final_vmdk_list
        
        # --- Der folgende Code bleibt als Fallback-Alternative vorhanden ---
        # VERBESSERTER ORPHANED VMDK SCAN (Alte Methode - nur verwaiste VMDKs):
        # Wir brauchen eine zuverlässigere Erkennung von tatsächlich verwaisten VMDKs
        # Strategie: Mehrschichtige Analyse aller Datastores mit direktem Zugriff
        
        # Erste Verbesserung: Direkter Scan nach allen VMDKs im Datastore und 
        # Vergleich mit den tatsächlich verwendeten (besser als zuvor)
        if False and len(orphaned_vmdks) == 0:
            logger.warning("VERBESSERTE ERKENNUNG: Direkter Datastore-Scan nach verwaisten VMDKs")
            
            # Alle bekannten VM-Pfade in eine Menge für effizientere Suche übertragen
            all_vm_paths = set()
            for disk in all_vm_disks:
                # Verschiedene Pfadvarianten für maximale Übereinstimmungsrate
                if 'path' in disk:
                    path = disk['path'].lower().strip()
                    all_vm_paths.add(path)  # Original-Pfad
                    
                    # Pfad ohne Datastore-Präfix
                    if '[' in path and ']' in path:
                        try:
                            path_without_ds = path.split(']', 1)[1].strip()
                            if path_without_ds:
                                all_vm_paths.add(path_without_ds)
                        except:
                            pass
                    ...
                    try:
                        ...
                        if base_name:
                            all_vm_paths.add(base_name)
                    except:
                        pass
                        
            logger.warning(f"Verbesserte Erkennung: {len(all_vm_paths)} Pfadvarianten bekannter VM-Disks werden beim Vergleich berücksichtigt")
                                                ...
                                                try:
                                                    if hasattr(file_info, 'path') and file_info.path:
                                                        ...
                                                        if file_path.endswith('.vmdk') and "-flat.vmdk" not in file_path:
                                                            # Prüfen, ob diese VMDK in bekannten VM-Pfaden vorkommt
                                                            full_vmdk_path = (result.folderPath + file_info.path).lower()
                                                            vmdk_name = file_info.path.lower()
                                                            
                                                            # PowerShell-ähnliche Logik zur Prüfung, ob eine VMDK verwaist ist
                                                            is_orphaned = True
                                                            
                                                            # Mehrere Varianten des Pfades prüfen
                                                            if full_vmdk_path in all_vm_paths:
                                                                is_orphaned = False
                                                            
                                                            # Prüfen, ob der VMDK-Name in irgendeinem bekannten Pfad vorkommt
                                                            if is_orphaned:
                                                                for vm_path in all_vm_paths:
                                                                    if vmdk_name in vm_path:
                                                                        is_orphaned = False
                                                                        break
                                                            
                                                            # Wenn kein Präfix in Pfad, betrachte es ohne Datastore-Teil
                                                            if is_orphaned and '[' in full_vmdk_path and ']' in full_vmdk_path:
                                                                try:
                                                                    no_ds_path = full_vmdk_path.split(']', 1)[1].strip()
                                                                    if no_ds_path in all_vm_paths:
                                                                        is_orphaned = False
                                                                except:
                                                                    pass
                                                            
                                                            if debug_mode:
                                                                logger.warning(f"FALLBACK CHECK: VMDK {full_vmdk_path} ist verwaist: {is_orphaned}")
                                                            
                                                            # Wenn die VMDK wirklich verwaist ist, hinzufügen
                                                            if is_orphaned:
                                                                orphaned_info = {
                                                                    'vm': "VERIFIZIERTE VERWAISTE VMDK",
                                                                    ...
                                                                    'status': "POTENTIALLY ORPHANED",
                                                                    'explanation': "POTENTIALLY ORPHANED: Diese VMDK-Datei wurde mit mehreren Methoden überprüft und scheint nicht mit einer aktiven VM verbunden zu sein. Bitte überprüfen Sie die Datei manuell."
                                                                }
                                                                orphaned_vmdks.append(orphaned_info)
                                                except Exception:
                                                    pass
